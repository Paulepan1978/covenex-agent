...
def auth_start():
    ...
    if not CONFIG["GMAIL_CLIENT_ID"] or not CONFIG["GMAIL_CLIENT_SECRET"]:
        return "Gmail Client ID oder Secret fehlt in den Environment Variables!", 400
    ...
    client_config = {
        "web": {
            ...
            "auth_uri": "https://accounts.google.com/o/oauth2/auth",
            "token_uri": "https://oauth2.googleapis.com/token",
            "redirect_uris": [f"https://covenex-agent.onrender.com/oauth/callback"]
        }
    }
    ...
    try:
        ...
        return f"Fehler beim Starten des OAuth Flows: {str(e)}", 500


@app.route("/oauth/callback", methods=["GET"])
def oauth_callback():
    """Empfängt den OAuth Callback von Google."""
    import flask
    from google_auth_oauthlib.flow import Flow
    ...
    client_config = {
        "web": {
            ...
            "auth_uri": "https://accounts.google.com/o/oauth2/auth",
            "token_uri": "https://oauth2.googleapis.com/token",
            "redirect_uris": ["https://covenex-agent.onrender.com/oauth/callback"]
        }
    }
    ...
    try:
        ...
        <h1 style="color:#27ae60">✓ Gmail verbunden!</h1>
        <p style="color:rgba(255,255,255,0.7)">Der Covenex Agent kann jetzt E-Mails senden.</p>
        <p style="margin-top:20px"><a href="/" style="background:#185FA5;color:white;padding:12px 24px;border-radius:8px;text-decoration:none">Zum Dashboard →</a></p>
        </body></html>
        """, 200
    except Exception as e:
        log.error(f"OAuth Callback Fehler: {e}")
        return f"OAuth Fehler: {str(e)}", 500
